...
unur_reset_errno();
n_tests_failed += (check_expected_NULL(TESTLOG,28,(unur_auto_new( distr )))==UNUR_SUCCESS)?0:1;
...
unur_reset_errno();
n_tests_failed += (check_expected_setfailed(TESTLOG,45,(unur_auto_set_logss(par,1)))==UNUR_SUCCESS)?0:1;
n_tests_failed += (check_errorcode(TESTLOG,45,UNUR_ERR_PAR_INVALID)==UNUR_SUCCESS)?0:1;
unur_par_free(par);
unur_distr_free(distr);
}
...
unur_reset_errno();
n_tests_failed += (check_expected_setfailed(TESTLOG,54,(unur_auto_set_logss(par,-1)))==UNUR_SUCCESS)?0:1;
n_tests_failed += (check_errorcode(TESTLOG,54,UNUR_ERR_PAR_SET)==UNUR_SUCCESS)?0:1;
unur_par_free(par);
unur_distr_free(distr);
}
...
/* TDR */
par = unur_tdr_new(distr);
n_tests_failed += (compare_sequence_par_start(TESTLOG,97,par,COMPARE_SAMPLE_SIZE)==UNUR_SUCCESS)?0:1;

unur_reset_errno();
/* AUTO */
par = unur_auto_new(distr);
n_tests_failed += (compare_sequence_par(TESTLOG,101,par,COMPARE_SAMPLE_SIZE)==UNUR_SUCCESS)?0:1;
unur_distr_free(distr);
}
...
n_tests_failed += (compare_sequence_par_start(TESTLOG,110,par,COMPARE_SAMPLE_SIZE)==UNUR_SUCCESS)?0:1;

unur_reset_errno();
/* AUTO */
par = unur_auto_new(distr);
n_tests_failed += (compare_sequence_par(TESTLOG,114,par,COMPARE_SAMPLE_SIZE)==UNUR_SUCCESS)?0:1;
unur_distr_free(distr);
}
...
par = unur_auto_new(distr);
gen = unur_init(par);
n_tests_failed += (compare_sequence_gen_start(TESTLOG,145,gen,COMPARE_SAMPLE_SIZE)==UNUR_SUCCESS)?0:1;
