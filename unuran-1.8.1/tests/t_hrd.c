...
unur_reset_errno();
n_tests_failed += (check_expected_NULL(TESTLOG,35,(unur_hrd_new( distr )))==UNUR_SUCCESS)?0:1;
...
unur_reset_errno();
n_tests_failed += (check_expected_NULL(TESTLOG,41,(unur_hrd_new( distr )))==UNUR_SUCCESS)?0:1;
...
unur_reset_errno();
/* hazard rate */
n_tests_failed += (check_expected_NULL(TESTLOG,48,(unur_hrd_new( distr )))==UNUR_SUCCESS)?0:1;
...
unur_reset_errno();
n_tests_failed += (check_expected_setfailed(TESTLOG,58,(unur_hrd_set_verify(par,1)))==UNUR_SUCCESS)?0:1;
...
unur_reset_errno();
n_tests_failed += (check_expected_setfailed(TESTLOG,66,(unur_hrd_set_verify(par,1)))==UNUR_SUCCESS)?0:1;
...
unur_reset_errno();
n_tests_failed += (check_expected_setfailed(TESTLOG,86,(unur_hrd_chg_verify(gen,1)))==UNUR_SUCCESS)?0:1;
n_tests_failed += (check_errorcode(TESTLOG,86,UNUR_ERR_GEN_INVALID)==UNUR_SUCCESS)?0:1;
unur_distr_free(distr);
unur_free(gen);
}
...
abort_if_NULL(TESTLOG, 96,    par );


unur_reset_errno();
n_tests_failed += (check_expected_NULL(TESTLOG,100,(gen = unur_init( par )))==UNUR_SUCCESS)?0:1;
n_tests_failed += (check_errorcode(TESTLOG,100,UNUR_ERR_GEN_CONDITION)==UNUR_SUCCESS)?0:1;
unur_free(gen);
unur_distr_free(distr);
}
...
/* default algorithm */
par = unur_hrd_new(distr);
n_tests_failed += (compare_sequence_par_start(TESTLOG,127,par,COMPARE_SAMPLE_SIZE)==UNUR_SUCCESS)?0:1;

unur_reset_errno();
/* default algorithm - verifying mode */
par = unur_hrd_new(distr);
unur_hrd_set_verify(par,1);
n_tests_failed += (compare_sequence_par(TESTLOG,132,par,COMPARE_SAMPLE_SIZE)==UNUR_SUCCESS)?0:1;

unur_reset_errno();
/* default algorithm - ignore invalid boundary */
unur_distr_cont_set_domain(distr,-10.,10.);
par = unur_hrd_new(distr);
n_tests_failed += (compare_sequence_par_start(TESTLOG,137,par,COMPARE_SAMPLE_SIZE)==UNUR_SUCCESS)?0:1;
unur_distr_free(distr);
}
