...
unur_reset_errno();
n_tests_failed += (check_expected_NULL(TESTLOG,39,(unur_cext_new( distr )))==UNUR_SUCCESS)?0:1;
...
unur_reset_errno();
n_tests_failed += (check_expected_setfailed(TESTLOG,50,(unur_cext_set_init(par,NULL)))==UNUR_SUCCESS)?0:1;
n_tests_failed += (check_errorcode(TESTLOG,50,UNUR_ERR_NULL)==UNUR_SUCCESS)?0:1;

unur_reset_errno();
n_tests_failed += (check_expected_setfailed(TESTLOG,53,(unur_cext_set_sample(par,NULL)))==UNUR_SUCCESS)?0:1;
...
unur_reset_errno();
n_tests_failed += (check_expected_setfailed(TESTLOG,61,(unur_cext_set_init(par,NULL)))==UNUR_SUCCESS)?0:1;
n_tests_failed += (check_errorcode(TESTLOG,61,UNUR_ERR_PAR_INVALID)==UNUR_SUCCESS)?0:1;

unur_reset_errno();
n_tests_failed += (check_expected_setfailed(TESTLOG,64,(unur_cext_set_sample(par,NULL)))==UNUR_SUCCESS)?0:1;
n_tests_failed += (check_errorcode(TESTLOG,64,UNUR_ERR_NULL)==UNUR_SUCCESS)?0:1;

unur_reset_errno();
n_tests_failed += (check_expected_setfailed(TESTLOG,67,(unur_cext_set_sample(par,check_sample)))==UNUR_SUCCESS)?0:1;
...
unur_reset_errno();
n_tests_failed += (check_expected_NULL(TESTLOG,80,(unur_cext_get_params(gen,(size_t)0)))==UNUR_SUCCESS)?0:1;
...
UNUR_GEN   *gen = NULL;
   ...
   gen = unur_init(par); 
abort_if_NULL(TESTLOG, 104,    gen );


unur_reset_errno();
n_tests_failed += (check_expected_reinit(TESTLOG,108,(unur_reinit( gen )))==UNUR_SUCCESS)?0:1;
unur_distr_free(distr);
unur_free(gen);
}
...
unur_cext_set_sample(par,exp1_sample);
gen = unur_init(par);
n_tests_failed += (compare_sequence_gen_start(TESTLOG,180,gen,COMPARE_SAMPLE_SIZE)==UNUR_SUCCESS)?0:1;
...
par = unur_cstd_new(distr);
gen = unur_init(par);
n_tests_failed += (compare_sequence_gen(TESTLOG,192,gen,COMPARE_SAMPLE_SIZE)==UNUR_SUCCESS)?0:1;
...
unur_cext_set_sample(par,expa_sample);
gen = unur_init(par);
n_tests_failed += (compare_sequence_gen_start(TESTLOG,235,gen,COMPARE_SAMPLE_SIZE)==UNUR_SUCCESS)?0:1;
